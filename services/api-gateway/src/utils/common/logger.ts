...
export class Logger {
  ...
  static async logResponseError(
    ...
    try {
      Logger.winstonLogger.log(`error`, message, {
        isCritical,
        type: 'RESPONSE_ERROR',
        status,
      ...
      Logger.winstonLogger.error(exception);
    } catch (winstonError) {
  ...
  static async logError(
    ...
    const timestamp = new Date().toISOString();
    try {
      Logger.winstonLogger.error(`CaughtError: ${message}`, {
        isCritical,
        appName: process.env.APP_NAME,
        timestamp,
        functionName,
  ...
  static async logMessage(
    ...
    try {
      Logger.winstonLogger.log('info', message, {
        timestamp,
        appName: process.env.APP_NAME,
        functionName,
        metaInfo,
